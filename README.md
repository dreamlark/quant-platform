...
| `web/src/pages/Monitor.tsx` | 运维监控页（数据/因子/模型/管线状态/市场情绪指数卡片 + 实时运行日志） |
| `web/src/pages/Data.tsx` | 数据管理页（全量导入/简报重算/数据库概览/数据源优先级/清空库） |
...
| 路由 | 页面 | 内容 |
|------|------|------|
| `/` | Dashboard | 看板汇总 + **运维控制卡片**（全量更新 / 仅生成简报 / 终止 / 自动运行 / 进度条）；顶部双日期展示「行情库最新」与「简报/信号日期」 |
...
| `/monitor` | Monitor | 运维监控（数据/因子/模型/管线/历史）+ **实时运行日志面板**（分级着色、1.5s 轮询、自动滚动） |
| `/data` | 数据管理 | 全量导入 / 仅重算简报 / 刷新概览；market.duckdb 与 analytics.duckdb 库状态、数据源优先级、清空库 |

所有页面暗色现代风格，实时读 FastAPI。

> **双日期说明**：看板顶部同时展示「行情库最新交易日」与「简报/信号日期」。若两者不一致，说明 `ingest` 已拉到更新数据但后续因子→融合→简报流水线尚未跑完，点「全量更新」跑完整 12 步流水线即可对齐。
>
> **更新按钮语义**：「全量更新」= 跑完整 12 步流水线（耗时较长）；「仅生成简报」= 仅重新生成 LLM 每日简报，不重跑因子/预测；运行中可点「终止」取消当前任务。
...
| GET | `/api/admin/logs` | 实时运行日志（步骤级状态/进度，前端每 1.5s 轮询） |
...
**Q9：看板「数据日期」和「行情库最新」不一致？**
A：正常。前者是简报/信号日期，后者是行情库 `daily_bars` 最新交易日。`ingest` 拉到新数据后，若因子→融合→简报流水线还没跑完，两者就会错位。点看板「全量更新」跑完 12 步流水线即对齐；也可用 `/data` 数据管理页单独「仅重算简报」。

**Q10：baostock 拉数据报 `'bool' object is not subscriptable`？**
A：baostock 在查询出错时 `rs.next()` 返回 `bool(False)` 而非行数据。已修复：适配器先检查 `error_code`，并防御 `rs.next()` 返回非 `list/tuple` 的情况（见 `sources/baostock_adapter.py`）。误报会记入运维监控的实时运行日志，不影响整条流水线。

**Q11：实时运行日志一直显示「执行中」，怎么判断是真在跑还是卡死了？**
A：两步防护。（1）**卡死检测**：每个步骤有预期最大耗时（ingest 10 分、llm 5 分、其余 2-3 分）；超过后后端心跳日志升级为 `WARN`（橙色）提示「可能卡死（网络/磁盘阻塞或死锁）」，运维监控「管线运行」卡片也会弹出黄色告警横幅，并给出当前步骤已运行分钟数。（2）**结果确认**：每个步骤成功后后端按目标日统计对应落库表行数，日志打印「结果确认：XXX 已落库 N 行」，若行数为 0 则打 WARN「结果异常」。若长时间无任何日志且状态停在 running，基本可判定卡死，建议检查数据源连接或重启后端后重新触发。
